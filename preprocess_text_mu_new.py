...
def read_clean_file(path):
    """
    Reads text files, filtering out empty lines and raw Europarl XML tags (like <CHAPTER...>, <P>, etc.)
    """
    cleaned_lines = []
    for file_path in iter_text_files(path):
        with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
            for line in f:
                line_str = line.strip()
                # Skip empty lines or structural Europarl XML tags
...
def make_parallel_dataset(src_path, trg_path, token_to_idx, trg_lang, min_len, max_len):
    ...
    for src_file, trg_file in paired_text_files(src_path, trg_path):
        src_lines = read_clean_file(src_file)
        trg_lines = read_clean_file(trg_file)

        # Instead of throwing a line mismatch error, zip pairs until the shorter file ends
        for src_raw, trg_raw in zip(src_lines, trg_lines):
            src = normalize_text(src_raw)
            trg = normalize_text(trg_raw)
...
def save_json(data, path):
    """Saves dataset arrays into formatted JSON files."""
...
def main():
    ...
    parser.add_argument("--save-json", action="store_true", default=True, help="Also save datasets in JSON format.")
    ...
    for path in tqdm(files.values(), desc="Processing files"):
        ...
        for line in tqdm(read_clean_file(path), desc=f"Reading {file_basename}", leave=False):
    ...
    for name, data in tqdm(datasets.items(), desc="Splitting and saving datasets"):
        ...
        if args.save_json:
            save_json(train_data, os.path.join(args.output_dir, f"train_{name}.json"))
            save_json(test_data, os.path.join(args.output_dir, f"test_{name}.json"))

        print(f"{name:10s} | train: {len(train_data):10,d} | test: {len(test_data):10,d}")
